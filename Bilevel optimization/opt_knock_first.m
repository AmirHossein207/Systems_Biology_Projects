...
modelx = changeRxnBounds(modelx, {'EX_glc__D_e','EX_o2_e'}, [-10,0], 'b');
fbaWT = optimizeCbModel(modelx);
succFluxWT = fbaWT.x(strcmp(modelx.rxns, 'EX_succ_e'));
...
fprintf('The production of ethanol before optimization is %.1f \n', etohFluxWT);
fprintf('The growth rate before optimization is %.1f \n', growthRateWT);
fprintf(['The production of other products such as succinate, formate, lactate and'...
         'acetate are %.1f, %.1f, %.1f and %.1f, respectively. \n'], ...
        succFluxWT, formFluxWT, lactFluxWT, acetFluxWT);
%% 
...
options = struct('targetRxn', 'EX_etoh_e', 'numDel', 3);
% We will impose that biomass be at least 50% of the biomass of wild-type
constrOpt = struct('rxnList', {{biomass}},'values', 0.4*fbaWT.f, 'sense', 'G');
...
while nIter < threshold
    ...
    succFluxM1 = optKnockSol.fluxes(strcmp(modelx.rxns, 'EX_succ_e'));
    ...
    if ~isempty(setM1)
        ...
        fprintf('\n');
        fprintf('The production of ethanol after optimization is %.2f \n', etohFluxM1);
        fprintf('The growth rate after optimization is %.2f \n', growthRateM1);
        fprintf(['The production of other products such as succinate, formate, lactate and acetate are' ...
                 '%.1f, %.1f, %.1f and %.1f, respectively. \n'], succFluxM1, formFluxM1, lactFluxM1, acetFluxM1);
